...
export const createQueueHost = (): QueueHost => {
  return {
    ...
    getCurrentItem: async () => useQueueStore.getState().getCurrentItem(),
    ...
    clearQueue: async () => useQueueStore.getState().clearQueue(),
    ...
    goToNext: async () => useQueueStore.getState().goToNext(),
    goToPrevious: async () => useQueueStore.getState().goToPrevious(),
    goToIndex: async (index: number) =>
      useQueueStore.getState().goToIndex(index),
    goToId: async (id: string) => useQueueStore.getState().goToId(id),
