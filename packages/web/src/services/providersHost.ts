...
const createProvidersHost = (): ProvidersHost => {
  ...
  return {
    register<T extends ProviderDescriptor>(provider: T): string {
      ...
      const storeState = useProvidersStore.getState();
      if (!storeState.getActive(provider.kind)) {
        storeState.setActive(provider.kind, provider.id);
      }
    ...
    unregister(providerId: string): boolean {
      ...
      const storeState = useProvidersStore.getState();
      if (storeState.getActive(current.kind) === providerId) {
        const fallback = firstOfKind(current.kind);
        if (fallback) {
          storeState.setActive(current.kind, fallback.id);
        } else {
          storeState.clearActive(current.kind);
        }
      }
    ...
    resolveActiveOnBootstrap() {
      ...
      PROVIDER_KINDS.filter(
        (kind) => !isRegistered(storeState.getActive(kind)),
      ).forEach((kind) => {
        const fallback = firstOfKind(kind);
        if (fallback) {
          storeState.setActive(kind, fallback.id);
        }
      });

      const pairedStreamingProviderId = pairedStreamingProviderIdFor(
        storeState.getActive('metadata'),
      );
      if (
        pairedStreamingProviderId &&
        storeState.getActive('streaming') !== pairedStreamingProviderId
      ) {
        storeState.setActive('streaming', pairedStreamingProviderId);
      }
